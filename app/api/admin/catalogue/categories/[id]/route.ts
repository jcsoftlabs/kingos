...
export async function PATCH(requete: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { statut, corps } = await apiBackendAuthentifie(`/api/admin/catalogue/categories/${id}`, { method: "PATCH", body: corpsClient });
...
export async function DELETE(_requete: Request, { params }: { params: Promise<{ id: string }> }) {
  const { id } = await params;
  const { statut, corps } = await apiBackendAuthentifie(`/api/admin/catalogue/categories/${id}`, { method: "DELETE" });
  return NextResponse.json(corps, { status: statut });
}
