...
export async function PATCH(requete: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { statut, corps } = await apiBackendAuthentifie(`/api/admin/catalogue/services/${id}`, {
    method: "PATCH",
    body: corpsClient,
  });
...
export async function DELETE(_requete: Request, { params }: { params: Promise<{ id: string }> }) {
  const { id } = await params;
  const { statut, corps } = await apiBackendAuthentifie(`/api/admin/catalogue/services/${id}`, { method: "DELETE" });
  if (corps.succes) revalidatePath("/services");
  return NextResponse.json(corps, { status: statut });
}
